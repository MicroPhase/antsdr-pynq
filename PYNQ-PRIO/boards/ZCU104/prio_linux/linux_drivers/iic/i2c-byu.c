...
static int xiic_i2c_probe(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev, "start: 0x%lx\n", res->start);	
	dev_err(&pdev->dev, "end: 0x%lx\n", res->end);	
	dev_err(&pdev->dev, "name: 0x%s\n", res->name);	
	dev_err(&pdev->dev, "flags: 0x%lx\n", res->flags);	
	dev_err(&pdev->dev, "desc: 0x%lx\n", res->desc);
...
static int xiic_i2c_remove(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev, "IIC REMOVE\n");
	return 0;
}

#if defined(CONFIG_OF)
static const struct of_device_id xiic_of_match[] = {
	{ .compatible = "byu,xps-iic-2.00.a", },
	{},
};
