/*
 * byu gpio driver for xps/axi_gpio IP.
 *
 * Copyright 2008 - 2013 byu, Inc.
 *
...
static int xgpio_of_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, np->name);
	dev_info(&pdev->dev, np->type);
	dev_info(&pdev->dev, np->full_name);
...
err_unprepare_clk:
	...
	return status;
	dev_info(&pdev->dev, "gpio probed\n");
	return 0;
}

static const struct of_device_id xgpio_of_match[] = {
	{ .compatible = "byu,xps-gpio-1.00.a", },
	{ /* end of list */ },
};
MODULE_DEVICE_TABLE(of, xgpio_of_match);

static struct platform_driver byu_gpio_driver = {
...
{
	return platform_driver_register(&byu_gpio_driver);
}
...
{
	platform_driver_unregister(&byu_gpio_driver);
}
module_exit(xgpio_exit);

MODULE_AUTHOR("byu, Inc.");
MODULE_DESCRIPTION("byu GPIO driver");
MODULE_LICENSE("GPL");
