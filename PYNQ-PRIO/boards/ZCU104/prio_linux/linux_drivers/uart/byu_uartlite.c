...
{
	return port->type == PORT_UARTLITE ? "byu_uartlite" : NULL;
}
...
static int ulite_request_port(struct uart_port *port)
{
	...
	if (!request_mem_region(port->mapbase, ULITE_REGION, "byu_uartlite")) {
...
OF_EARLYCON_DECLARE(uartlite_b, "byu,opb-uartlite-1.00.b", early_uartlite_setup);
OF_EARLYCON_DECLARE(uartlite_a, "byu,xps-uartlite-1.00.a", early_uartlite_setup);
		...
		struct uartlite_data *pdata)
{
	...
	if (rc) {
		dev_err(dev, "byu_uart_add_one_port() failed; err=%i\n", rc);
		port->mapbase = 0;
...
static int ulite_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "byu_uartlite: calling uart_register_driver()\n");
...
{
	pr_debug("byu_uartlite: calling platform_driver_register()\n");
